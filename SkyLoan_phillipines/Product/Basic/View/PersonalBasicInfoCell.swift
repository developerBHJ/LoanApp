...
import UIKit
class PersonalBasicInfoCell: UITableViewCell {
...
extension PersonalBasicInfoCell{
    func setupUI(){
    ...
    private func creatStackView()-> UIStackView? {
        ...
        let itemViews = model.items.map { model in
            let view = PersonalBasicInfoItemView.init(frame: .zero,model: model)
            return view
        }
...
extension PersonalBasicInfoCell{
    struct Model {
        var items: [PersonalBasicInfoItemView.Model] = []
    }
}
